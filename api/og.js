...
export default async function handler(req, res) {
  try {
    ...
    let title = body.match(/<title>([^<]+)<\/title>/)?.[1].replace(/ - Tailwind CSS$/, '')
    ...
    let description = body.match(/<meta property="og:description" content="([^"]+)"/)?.[1]
    let html = getHtml({ title, description })
