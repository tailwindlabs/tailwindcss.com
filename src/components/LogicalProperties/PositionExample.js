...
export function PositionExample({ direction, hue = 'emerald' }) {
  return (
    <>
      <div class="flex justify-center font-mono text-white text-sm font-bold leading-6 py-8">
        <motion.div
          ...
          class={`bg-${hue}-500 bg-stripes-${hue} rounded-lg  w-3/4 relative h-32`}
        >
          <motion.div
            ...
            layout="position"
            class={`bg-${hue}-500 p-4 shadow-lg rounded-md absolute start-0 top-1/2`}
          >
            start-0
          </motion.div>
          <motion.div
            ...
            transformTemplate={(_, generated) => `translateY(-50%) ${generated}`}
            layout="position"
            class={`bg-${hue}-500 p-4 shadow-lg rounded-md absolute end-0 top-1/2`}
          >
            end-0
          </motion.div>
        </motion.div>
