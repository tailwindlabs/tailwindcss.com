...
export function PartnerDirectory({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    for (const item of items) {
      ...
      const visible =
        activeCategory === "All" ? true : categories.split(",").includes(activeCategory);
      item.hidden = !visible;
    }
  }, [activeCategory]);
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-[3fr_7fr] lg:items-start lg:gap-20">
        <div>
          ...
          <p className="mt-8 text-lg/7 font-medium tracking-tight text-pretty">
            ...
            <span className="text-gray-500 dark:text-gray-400">Browse the companies that make Tailwind CSS possible.</span>
          </p>
        </div>
        <div>
