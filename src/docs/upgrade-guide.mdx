...
Migrating from PostCSS is as simple as removing the postcss.config.js file and removing the autoprefixer, postcss packages. The @tailwindcss/postcss package is no longer required if it was added by the upgrade tool.
