...
The `transition` and `transition-color` utilities will now include `outline-color` in their property lists. To fix issues with transitioning e.g. on focus, ensure that `outline-color` is applied unconditionally:
