...
Learn more about automatic content detection in the [detecting classes in source files documentation](/docs/how-classes-are-generated).

<h3 id="utility-directive">@utility</h3>

Use the `@utility` directive to add custom utilities that work with variants like `hover`, `focus` and `lg`:
...
Custom utilities must be a single class name and not a complex selector. If your custom utility is more complex than a single class name, use nesting to define the utility:
...
Use the `--alpha()` function to generate a color with an opacity applied to it:
...
This function is deprecated, and we recommend referencing [theme variables](/docs/theme) instead.
