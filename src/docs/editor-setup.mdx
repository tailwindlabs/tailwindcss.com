...
Zed offers builtin support for Tailwind CSS autocomplete, linting, and hover previews for a variety of languages. It additionally offers support for Prettier so our official [Prettier plugin](https://github.com/tailwindlabs/prettier-plugin-tailwindcss) will also work with Zed when it is installed.
