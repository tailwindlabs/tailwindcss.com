...
[Cursor](https://cursor.sh/?utm_source=tailwind) is a VS Code–compatible editor with built-in AI features. Since it’s based on VS Code, all of the Tailwind CSS tooling you’re already familiar with works out of the box — including our official Tailwind CSS IntelliSense extension and the Prettier plugin for class sorting.

<img
  src={require("./img/cursor-intellisense.png").default.src}
  alt="Tailwind CSS IntelliSense working in Cursor"
  width={1344}
  height={672}
  className="rounded-lg"
/>

Learn more about [Cursor](https://cursor.com/) and check out the [docs](https://cursor.com/docs).
