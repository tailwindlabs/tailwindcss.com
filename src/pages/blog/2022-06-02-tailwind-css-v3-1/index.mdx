...
Then on a random Saturday night a couple of weeks ago, I was talking to Robin in our Discord about coming up with a way to target the `html` element using `:has` and a class deeper in the document, and explained how I thought it would look if we added support for arbitrary modifiers — something I've wanted to tackle for over a year:

<img src={discordMessage} alt="" />

Twenty minutes later Robin had a working proof of concept _(in six lines of code!)_, and after another hour or so of Jordan performing regex miracles in our class detection engine, [arbitrary modifiers](https://github.com/tailwindlabs/tailwindcss/pull/8299) were born and we had our release-worthy feature.
...
Okay so this one is the real highlight for me — you know how we give you the [`addVariant` API](https://tailwindcss.com/docs/plugins#adding-variants) for creating your own custom modifiers?
...
Well Tailwind CSS v3.1 introduces [arbitrary modifiers](/docs/hover-focus-and-other-states#using-arbitrary-modifiers), letting you create your own ad hoc modifiers directly in your HTML:
...
This is super useful for modifiers that sort of feel like they need to be parameterized, for example adding a style if the browser supports a specific CSS feature using a `@supports` query:
...
You can even use this feature to target child elements with arbitrary modifiers like `[&>*]`:
...
I've already got a bunch of ideas for Tailwind CSS v3.2 _(maybe even text shadows finally?!)_, but right now we're working hard to push these new website templates over the finish line. Look for another update on that topic in the next week or two!