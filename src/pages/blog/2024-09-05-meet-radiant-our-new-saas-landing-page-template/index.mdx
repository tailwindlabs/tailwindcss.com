...
To prove how reusable these elements are, we built a second landing page using the same components but with a slightly different look and feel, this time re-imagined with the [SavvyCal](https://savvycal.com/) brand:
...
What's even better is that non-developers are able to manage the blog directly from the Sanity Studio control panel, without having to write any code or push a deployment. Sanity Studio exists as a route in your app meaning you don't need go to the Sanity website to edit your blog content.
...
They also have this handy hot spot feature that lets you define a focal point for an image, so that when the image is cropped, it crops around the focal point. This is super useful for images that have a subject that you want to keep in frame.

![Sanity image hot spot tool](./sanity-hotspot.png)
