...
Use `h-dvh` to make an element span the entire dynamic height of the viewport, which changes as the browser UI expands or contracts.

<DynamicViewportExample unit='dvh' colorStyles='dark:bg-blue-500 bg-blue-500 border border-blue-400' containerBackground='bg-stripes-blue'/>
...
Use `h-lvh` to set an element's height to the largest possible height of the viewport, calculated when the browser UI is at its smallest. This behaves the same as `100vh`.
...
Use `h-svh` to set an element's height to the smallest possible height of the viewport, calculated when the browser UI is at its largest.
