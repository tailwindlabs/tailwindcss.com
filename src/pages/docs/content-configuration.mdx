...
For symlinked packages (e.g. when using Turborepo with a shared UI library), Tailwind will only read the content directory on the initial build. To get hot reloading working, you might need to resolve the linked package manually:
...
```js tailwind.config.js
  ...
  module.exports = {
    content: [
      './components/**/*.{html,js}',
      './pages/**/*.{html,js}',
      // Hot reloading doens't work with symlinks. realpathSync will resolve the full path of the linked dependency
>    path.join(require("fs").realpathSync('./node_modules/ui/'), 'src/**/*.{ts,tsx}'),
    ],
    // ...
  }
```
