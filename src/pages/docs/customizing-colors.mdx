...
Then define your colors in your configuration file using the `rgb` and/or `hsl` helpers that allow you to use these variables:
...
These functions require that your css variables use space separated values without
the alpha channel because Tailwind will add those based on other utilities.

If you need more control, then you can use the verbose signature with _functions_, and apply the `opacityValue` if it's defined:
