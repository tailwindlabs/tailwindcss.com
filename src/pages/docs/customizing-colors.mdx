...
Then define your colors in your configuration file using the `rgb` or `hsl` helpers, which will wrap your variables with the right color function and automatically insert the alpha channel whenever you use an opacity modifier:
...
When defining your colors this way, make sure that your CSS variables are using the modern [space-separated syntax](https://css-tricks.com/the-expanding-gamut-of-color-on-the-web/#aa-a-tale-of-new-syntaxes), and not the legacy comma-separated syntax:

<TipGood>Define your CSS variables using the space-separated syntax</TipGood>
...
<TipBad>Don't use the legacy comma-separated syntax</TipBad>
...
If you need more control or need to use a legacy color format, you can also manually define your colors as _functions_ which receive an `opacityValue` you can incorporate when needed:
...
You can extract your own helper function with whatever custom logic you need to make this less verbose:

```js tailwind.config.js
function rgba(variable) {
  return ({ opacityValue }) => {
    if (opacityValue === undefined) {
...
module.exports = {
  theme: {
    colors: {
      primary: rgba('--color-primary'),
      secondary: rgba('--color-secondary'),
      // ...
    }
  }
}
