...
### Interactions with responsive font size classes

Since v2.0, `text-{size}` classes are pre-defined with accompanying line-height definitions. See the [responsive documentation for `text-{size}`](/docs/font-size#interactions-with-line-height-classes) for a note about interactions with `leading-{size}` classes.
