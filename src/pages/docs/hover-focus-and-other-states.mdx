...
Style the backdrop of a native dialog using the `backdrop` modifier. Note: the `::backdrop` will only show up if you opened the dialog via `dialog.showModal()`.

<SnippetGroup>

```html index.html
<dialog id="my-dialog" class="**backdrop:bg-gray-50**" dark-class="**backdrop:bg-gray-50**">
  I'm a styled dialog
</dialog>
```
...
This can be used in combination with the [`open`](/docs/hover-focus-and-other-states#open-closed-state) modifier.
...
The `prefers-contrast` media query tells you if the user has requested the content to be presented
...
<Example hint="Try emulating `prefers-contrast: more` in your developer tools to increase the contrast between background and text color">
  <div class="flex items-center justify-center">
    <button type="button" class="inline-flex items-center px-4 py-2 font-black leading-6 text-sm shadow rounded-md bg-indigo-600 text-indigo-200 contrast-more:text-white" disabled>
      Save changes
    </button>
  </div>
</Example>
...
```html
<button type="button" class="bg-indigo-600 **contrast-more:text-white** ..." disabled>
  Save changes
</button>
```

Use the `contrast-less` modifier to conditionally add styles when the user has requested less contrast.
