...
If you don't want to create a plugin for a one-off use case, then you can also use arbitrary
variants. These are variants that are defined inline in your html.
...
This will translate into the typical "children" variant where each child of the `ul` will now get
the `underline` treatment.

You can also use at-rules like media queries just like how you can do it in the `addVariant` and
`matchVariant` APIs.

```js index.html 
<div class="[@supports(hover:hover)]:hover:underline">
  <!-- ... -->
</ul>
```
