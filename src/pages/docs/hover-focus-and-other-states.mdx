...
It can also be combined with the `peer-*` modifier to target a previous sibling with `peer-has-`.
...
<div class=' max-w-md mx-auto bg-white shadow py-6 px-4'>
  <fieldset class="grid">  
    ...
    <div class="grid grid-cols-[auto_1fr] gap-4 items-center pt-3">  
      <input class="peer accent-pink-500 rounded-sm appearance-none checked:appearance-auto border size-3.5 border-slate-300 dark:border-slate-500" id="item_1" checked type="checkbox">
      <label for="item_1" class="peer-has-[:checked]:text-blue-500 peer-has-[:checked]:line-through text-slate-700">Testing</label>    
    </div>
  </fieldset>
</div>
```
