...
### Styling based on child state <small>(has-{'{modifier}'})</small>

Use the `has-*` modifier to style a parent based on the state or content of a child element.
...
<div class=' max-w-md mx-auto bg-white shadow py-6 px-4'>
  <fieldset class="grid w-full items-start gap-1">
    ...
    <label for="apple-1" class="text-slate-700 has-[:checked]:ring-indigo-500 has-[:checked]:bg-indigo-50 grid h-[64px] grid-cols-[24px_1fr_auto] items-center gap-6 rounded-lg px-4 ring-1 ring-transparent hover:bg-slate-100">
      ...
      Google Pay
      <input name="paymentMethod" id="apple-1" value="google" type="radio" class="box-content h-1.5 w-1.5 appearance-none rounded-full border-[5px] border-white bg-white bg-clip-padding outline-none ring-1 ring-gray-950/10 checked:border-indigo-500 checked:ring-white" />
    </label>
...
The `has-` modifier can also be used with a combinator, like `~` or `+`, to style an element based on specific siblings.


```html {{ example: { p: 'none' } }}
<div class="mx-auto max-w-md bg-white px-4 py-6 shadow">
  <fieldset class="grid w-full grid-cols-2 items-start gap-4">
    <legend class="mb-4 text-slate-500">Select shipping method:</legend>
    <label for="standard" class="has-[:checked]:ring-blue-400 has-[:checked]:bg-blue-50 grid grid-cols-[1fr_auto] gap-y-3 rounded-lg p-3 text-slate-700 ring-1 ring-slate-100 hover:ring-slate-200">
      <div class="grid">
        <span class="font-medium">Standard</span>
        <span class="text-sm text-slate-500">5-10 business days</span>
      </div>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" class="has-[~:checked]:text-blue-500 h-6 w-6 text-slate-500">
        <path d="M4.74994 8L11.9999 4.75L19.2499 8L11.9999 11.25L4.74994 8Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M4.74994 16L11.9999 19.25L19.2499 16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M19.2499 8V16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M4.74994 8V16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M11.9999 11.5V19" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
      </svg>
      <input name="shippingMethod" id="standard" value="google" type="radio" class="sr-only appearance-none" checked />
      <div class="text-sm font-semibold">Free</div>
    </label>
    <label for="express" class="has-[:checked]:ring-blue-400 has-[:checked]:bg-blue-50 grid grid-cols-[1fr_auto] gap-y-3 rounded-lg p-3 text-slate-700 ring-1 ring-slate-100 hover:ring-slate-200">
      <div class="grid">
        <span class="font-medium">Express</span>
        <span class="text-sm text-slate-500">2-4 business days</span>
      </div>
      <svg class="has-[~:checked]:text-blue-500 w-6 text-slate-500" viewBox="0 0 24 24" fill="none">
        <path d="M18.25 13C18.25 16.4518 15.4518 19.25 12 19.25C8.54822 19.25 5.75 16.4518 5.75 13C5.75 9.54822 8.54822 6.75 12 6.75C15.4518 6.75 18.25 9.54822 18.25 13Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
      ...
      <input name="shippingMethod" id="express" value="google" type="radio" class="sr-only appearance-none" />
...
```html
<label class="... has-[:checked]:ring-blue-500 has-[:checked]:bg-blue-50">
  <!-- ... -->
  <svg class="... **has-[~:checked]:text-blue-500**">
    <!-- ... -->
  </svg>
  <input type="radio" class="sr-only appearance-none" />
</label>
```

The `has-*` modifier can also be used with the `peer-*` modifier to target a previous sibling.

```html {{ example: { p: 'none' } }}
<div class=' max-w-md mx-auto bg-slate-800 shadow py-6 px-4'>
  <fieldset class="grid">  
    <legend>Shopping List:</legend>
    <div class="grid grid-cols-[auto_1fr] gap-4 items-center pt-3">  
      <input class="peer accent-pink-500 rounded-sm appearance-none checked:appearance-auto border size-3.5 border-slate-500" id="item_1" checked type="checkbox">
      <label for="item_1" class="peer-has-[:checked]:text-blue-500 peer-has-[:checked]:line-through text-gray-100">Testing</label>    
    </div>
  </fieldset>
</div>
```
