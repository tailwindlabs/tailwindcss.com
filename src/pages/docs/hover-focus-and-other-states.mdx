...
**出于文件大小的考虑，并不是所有的状态都默认为所有的工具类启用**，但是我们已经尽了最大的努力来启用最常用的组合。

有关默认启用的状态的完整列表，请查看本页末尾的[参考表](#default-variants-reference)。

如果你需要一个Tailwind不支持的状态，你可以通过[编写一个状态插件](#creating-custom-variants)来扩展。
...
注意，与大多数其他变体不同的是，`motion-safe`可以与响应式前缀以及其他状态(如`hover`)结合在一起，按以下顺序堆叠它们:
...
注意，与大多数其他变体不同的是，`motion-reduce`可以与响应式前缀以及其他状态(如`hover`)结合在一起，按以下顺序堆叠它们:
...
状态也具有响应性，这意味着你可以做一些事情，比如在不同的断点更改元素的hover样式。

要在特定的断点上应用一个状态，在状态前缀之前先添加响应式前缀:
...
你可以为你自己的CSS类生成状态，用`@variant`指令包装它们:
...
你可以创建Tailwind默认不支持的状态，方法是编写一个自定义的变量插件。

例如，这个简单的插件增加了对`required`伪类状态的支持:
...
更多细节，查看[状态插件](/docs/plugins#adding-variants).

---

## 默认状态支持

由于文件大小的考虑，默认情况下，Tailwind并不包括所有工具类的所有状态。

要为你的项目配置启用哪些状态，查看[配置状态](/docs/configuring-variants).
