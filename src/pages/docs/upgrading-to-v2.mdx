...
Be sure to add a `{screen}:leading-{size}` for each responsive `{screen}:text-{size}` you have as well. See the [explanation in the `text-{size}` responsive docs](/docs/font-size#interactions-with-line-height-classes).
