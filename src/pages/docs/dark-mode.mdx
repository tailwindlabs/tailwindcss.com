...
为了使这变得尽可能简单，Tailwind包含了一个`dark`状态，当启用黑色模式时，你可以用不同的风格来设计你的网站:
...
需要格外注意的是，由于文件大小的考虑，默认情况下，Tailwind不启用暗模式状态。
...
`dark`可以跟[响应式前缀](/docs/responsive-design)以及其他[状态](/docs/hover-focus-and-other-states)叠加：
...
响应式前缀必须在第一位，然后是`dark`，最后是其他状态。

---

## 为其他工具类开启

要开启`dark`，方法跟其他状态一样，在配置文件添加即可：
