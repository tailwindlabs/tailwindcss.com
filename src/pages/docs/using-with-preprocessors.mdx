...
- **您的构建将更快**。因为你的CSS不需要被多种工具解析和处理，你的CSS只使用PostCSS会编译得更快。
...
这个问题的最简单的解决方案是永远不要在同一个文件中写常规CSS和导入其他CSS文件。相反，为您的导入创建一个主要入口点文件，并将所有实际的CSS保存在单独的文件中。
...
您最有可能遇到这种情况的地方是在主CSS文件中，其中包括您的`@tailwind`声明。
...
您可以使用[postcss-preset-env](https://github.com/csstools/postcss-preset-env)插件为您的项目添加数十个即将到来的CSS特性。
...
要使用Tailwind和预处理工具(如Sass、Less或Stylus)一起使用，您需要向项目添加额外的构建步骤，让您通过PostCSS运行预处理后的CSS。如果您在项目中使用Autoprefixer，那么您已经设置了类似的东西。
...
为了更好结合具体开发过程，建议您[专门使用PostCSS](#post-css)。
