...
You can change the default gray scale from `prose-gray` to one of the other supplied colors or to your own color theme, read the documentation on [changing the default gray scale](#changing-the-default-gray-scale)
...
You can change the default type scale from `prose-base` to one of the other supplied sizes, read the documentation on [changing the default type scale](#changing-the-default-type-scale)
...
### Changing the default gray scale

If you want to set the default gray scale to be something other than `prose-gray`, you can do so using the `color` option when registering the plugin:
...
The supplied color options are:

| color              | Class          |
| ------------------ | -------------- |
| `gray` _(default)_ | `prose-gray`   |
| `slate`            | `prose-late`   |
| `zinc`             | `prose-inc`    |
| `neutral`          | `prose-eutral` |
| `stone`            | `prose-tone`   |

You can also set the default to the name of your own custom color theme which you have defined in the config. In the [example above](#adding-custom-color-themes), the color name is `pink`. So you could set that color as the default when registering the plugin like this:
...
### Changing the default type scale

If you want to set the default type scale to be something other than `prose-base`, you can do so using the `scale` option when registering the plugin:
...
The supplied scale options are:

| scale              | Class        |
| ------------------ | ------------ |
| `sm`               | `prose-sm`   |
| `base` _(default)_ | `prose-base` |
| `lg`               | `prose-lg`   |
| `xl`               | `prose-xl`   |
| `2xl`              | `prose-2xl`  |

<TipInfo>This will only change the default type scale, if you want your text to scale at different break points, you will still need to utilise Tailwind's [breakpoint modifiers](/docs/responsive-design), read the documentation on [applying a type scale](#applying-a-type-scale)</TipInfo>
