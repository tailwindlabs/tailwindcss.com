...
If you'd like to apply a utility at one breakpoint only, combine a responsive modifier with a `max` modifier to prevent that utility from being applied on larger screen sizes.

Here is an example where the background color is red at the `md` breakpoint, but green at every other breakpoint:

```html
<div class="bg-green-500 md:max-lg:bg-red-500">
  <!-- ... -->
</div>
```
...
If you'd like to apply a utility for a specific breakpoint range, combine a responsive modifier with a `max` modifier.

Here is an example where the background color is red from the `sm` breakpoint up until the `lg` breakpoint, and green at every other breakpoint:

```html
<div class="bg-green-500 sm:max-lg:bg-red-500">
  <!-- ... -->
</div>
```
