...
Use the `line-clamp-*` utilities to specify how many lines of text should be visible before truncating.
...
The `line-clamp-none` utility only resets the `-webkit-line-clamp` property and does not remove the `overflow: hidden` and `display: -webkit-box` values. These properties need to be manually reset.
