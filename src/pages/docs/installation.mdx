...
如果您从未听说过PostCSS，或者想知道它与Sass等工具有何不同，请阅读[使用PostCSS当做预处理器](/docs/using-with-preprocessors#using-post-css-as-your-preprocessor)。
...
如果您不确定您使用的工具的PostCSS插件配置在哪里，您可以参考这些工具的文档，查看应该放在哪里。 直接Google搜索"configure postcss {my tool}"也可以很快得到答案。
...
为了让顺风的样式像预期的那样工作，你需要使用HTML5 doctype并包括响应视口元标签来正确处理所有设备上的响应样式。
