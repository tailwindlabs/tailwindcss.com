...
One additional limitation of arbitrary values is they can't include spaces, so be careful to replace them with an appropriate character.

<TipBad>Don't use spaces between `[` and `]`</TipBad>
...
<TipGood>Substitute spaces with an appropriate character or no character at all.</TipGood>
