...
This is an exciting week at Tailwind Labs. For one, it's my one year anniversary (🥳) but more importantly: We're shipping something that fundamentally changes how you can use Tailwind Plus.
...
`<el-listbox>` implements a form-associated custom element, so accessing it's value in a submitted form works just like native `<select>` elements:
...
<%= form_with do |form| %>
  ...
  <el-listbox name="car_id" id="car_id" value="<%= @selected_car.id %>">
    <button type="button">
      <el-selectedcontent>
        <%= @selected_car.name %>
      </el-selectedcontent>
